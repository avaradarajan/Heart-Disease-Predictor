...
def returnMatch(line):
    group = re.search(r'^.*\b(mm+|oh+|ah+|umm*|hmm*|huh|ugh|uh|sigh+)\b(.)*\b.*$',str(line[1]),0|re.I)
    if (group):
        print(f'{line[0]} {group.group(1)} {line[1].split(group.group(1))[1].strip()}')
        return [group.group(1),re.sub(r"[\\?+ | \\:+ | \\*+ | \\#+ | \\@+ | \\;+ | \\,+ | \\)+ | \\(+]"," ",line[1].split(group.group(1))[1]).strip()];
