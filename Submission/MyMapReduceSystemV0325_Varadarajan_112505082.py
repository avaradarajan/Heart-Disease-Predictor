...
class MyMapReduce:
        ...
        if combiner:
            ...
            for k, vs in valPerK.items():
                if vs:
                    ...
                    if(fromIntermediateReducer):
                        if(type(fromIntermediateReducer) is not int): #Combiner For word count
                            key = fromIntermediateReducer[0]
                            value = fromIntermediateReducer[1]
                            namenode_m2r.append((self.partitionFunction(key), (key, value)))
                        else: # Combiner for Set DIfference
                            print("Set Difference cannot work with combiner as a particular subset may not have elements from S")
                            break
        else:
            for (k, v) in mapped_kvs:
    ...
    def runSystem(self):
        ...
        # the output of map and reduce steps, respectively.
        ...
        # and creates child process for each of the map core available to start the map step i.e a set of map tasks.
        ...
        ##[SEGMENT 4]
        # What: This segment creates an array of empty lists based on the number of reducer cores specified. Based on the reducer
        # assignment from the map step, each of the inner lists are populated with a list of key value pairs (word,count).
        # Why: This step is a preprocessing step to ensure delegation and distribution of the workload to each of the reducer.
        #
        ...
        # [SEGMENT 5]
        # What: This segment extracts each of the list elements inside the to_reduce_task and creates a process to start the
        # reduction process i.e. a set of reduce tasks. The result from reduce step is populated in the namenode_fromR list.
        # Why: This step is needed to parallelize the process of reduction to achieve data processing with comparatively less time.
        #
...
if __name__ == "__main__":
    ...
    test1 = [('R', []), ('S', [1, 2])]
    test2 = [('R', [1, 3, 5]), ('S', [2, 4])]
    test3 = [('R', range(1, 5)), ('S', range(2, 6))]
    mrObject = SetDifferenceMR(test1, 2, 2)
    mrObject.runSystem()
    mrObject = SetDifferenceMR(test2, 2, 2)
    mrObject.runSystem()

    #invalid scenario as we cannot use combining step for set difference example
