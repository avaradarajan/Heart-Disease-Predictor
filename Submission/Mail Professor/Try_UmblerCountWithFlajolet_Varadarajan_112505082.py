...
import os #for ensuring file path works in all machines and not absolute

path = (str(os.getcwd()).replace("\\", "//")) #for ensuring file path works in all machines and not absolute
...
def returnMatch(line):
    currentList = []
    for m in re.finditer(r'\b(mm+|oh+|ah+|um+|hm+|huh|ugh|uh|sigh|sighed|sighing|sighs)\b', line,0|re.I):
...
def umbler(sc, rdd):
    ...
    locationRDD = sc.textFile(path+'//umbler_locations.csv').map(lambda inp: inp.replace('"', "")).map(
        lambda x: (x, ''))
...
def runTests(sc):
    ...
    #Umbler Tests:
    print("\n*************************\n Umbler Tests\n*************************")
    testFileSmall = path+'//publicSampleLocationTweet_small.csv'
    testFileLarge = path+'//publicSampleLocationTweet_large.csv'
...
#Use this if not calling from spark shell
