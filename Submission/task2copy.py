...
def returnMatch(line):
    ...
    if (group):
        print(f'{group.group(1)} {line.split(group.group(1))[1][1:].strip()}--')
        return [group.group(1),line.split(group.group(1))[1][line.split(group.group(1))[1].find(" "):].strip()];
    ...
    def countDistinctUsingStreamingAlgorithm(d):
        ...
        if (list is not None):
            ...
            for word in words[:3]:
                if (word):
                    wordToBeHashed += word;
            wordToBeHashed += nonFluencyDictionary.get(list[0][:2].lower())
