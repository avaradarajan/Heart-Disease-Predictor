...
def umbler(sc, rdd):
    ...
    locationRDD = sc.textFile(path+'//convertcsv.csv').map(lambda inp: inp.replace('"', "")).map(
        lambda x: (x, ''))
...
#Uncomment this if you are not using pyspark shell for running the program
sc = SparkContext(master="local[*]",appName="UmblerApp")
runTests(sc)
sc.stop()
