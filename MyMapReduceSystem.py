...
class MyMapReduce:
    ...
    def __init__(self, data, num_map_tasks=5, num_reduce_tasks=3, use_combiner=False):
        ...
        self.user_combiner = use_combiner  # whether or not to use a combiner within the map task.
...
def createSparseMatrix(X, label):
    label = label + ':' + ','.join([str(k) for k in np.array(X).shape])
...
if __name__ == "__main__":  # [Uncomment peices to test]
    ...
    '''
    print("\n\n*****************\n Matrix Multiply\n*****************\n")
    test1 = [(('A:2,1', 0, 0), 2.0), (('A:2,1'. 0, 1), 1.0), (('B:1,2', 0, 0), 1), (('B:1,2', 1, 0), 3)   ]
    test2 = createSparseMatrix([[1, 2, 4], [4, 8, 16]], 'A').\
        extend(createSparseMatrix([[1, 1, 1], [2, 2, 2], [4, 4, 4], 'B'))
    test3 = createSparseMatrix(np.random.randint(-10, 10, (10,100)), 'A').\
        extend(createSparseMatrix(np.random.randint(-10, 10, (100,12)), 'B'))
